...
export default async function handler(req, res) {
  ...
  try {
    ...
    const resultado = finalizados.map(m=>{
      const h=m.homeTeam?.name||"", a=m.awayTeam?.name||"";
      const jogo=achar(h,a);
      return {
        api: h+" x "+a,
        normalizado: norm(h)+" x "+norm(a),
        casou: jogo ? "SIM id="+jogo.id : "NAO",
        total_jogos_no_array: JOGOS.length
      };
    });
    return res.status(200).json({finalizados: finalizados.length, resultado});
  } catch(err){
