// ============================================================
//  BOLÃO DO TONI — Copa do Mundo 2026  ·  VERSÃO DEFINITIVA
//  Inclui: logo, nome Bolão do Toni, senhas por camisa,
//  trocar senha, painel admin, busca automática, bloqueio 1h
// ============================================================
...
const KEY_EXTRA_PARTS = "bolao_extra_participantes";
...
function getRankingData(palpites, resultados) {
  return getParticipantes().map(nome => {
    let total=0,exatos=0,acertos=0;
    JOGOS.forEach(j => {
      const pal=palpites[nome]?.[j.id], res=resultados[j.id];
      if(!pal||!res) return;
...
function LoginView({setParticipante,setView,participante}) {
  const [selected,setSelected]=useState(""); const [senha,setSenha]=useState(""); const [err,setErr]=useState("");
  function handleLogin() {
    ...
    setParticipante(selected); setView("palpites");
  }
  function handleLogout(){localStorage.removeItem(KEY_SESSION);setParticipante(null);setView("home");}
  return (
    <div className="view-container">
      <div className="card login-card">
        ...
        <select className="select-input" value={selected} onChange={e=>{setSelected(e.target.value);setErr("");}}>
          ...
          {getParticipantes().map(n=><option key={n} value={n}>{n}</option>)}
        </select>
        <input className="select-input" type="password" placeholder="Sua senha"
          value={senha} onChange={e=>{setSenha(e.target.value);setErr("");}} onKeyDown={e=>e.key==="Enter"&&handleLogin()}/>
        {err&&<p className="error-msg">⚠️ {err}</p>}
        <button className="btn-primary btn-full" onClick={handleLogin} disabled={!selected||!senha}>Entrar →</button>
...
function TrocarSenhaView({setView}) {
  const [selected,setSelected]=useState(""); const [senhaAtual,setSenhaAtual]=useState("");
  const [novaSenha,setNovaSenha]=useState(""); const [confirmar,setConfirmar]=useState("");
  const [err,setErr]=useState(""); const [sucesso,setSucesso]=useState(false);
  function handleTrocar() {
    if(!selected){setErr("Selecione seu nome.");return;}
    if(!verificarSenha(selected,senhaAtual)){setErr("Senha atual incorreta!");return;}
    if(novaSenha.length<3){setErr("Nova senha precisa ter ao menos 3 caracteres.");return;}
    if(novaSenha!==confirmar){setErr("As senhas não coincidem.");return;}
    const customs=loadStorage(KEY_MINHAS_SENS,{}); customs[selected]=novaSenha;
    localStorage.setItem(KEY_MINHAS_SENS,JSON.stringify(customs));
    setSucesso(true); setTimeout(()=>setView("login"),2500);
  }
  if(sucesso) return (
    <div className="view-container"><div className="card login-card">
      <div className="card-icon">✅</div><h2 className="card-title">Senha alterada!</h2>
      <p className="card-sub">Redirecionando...</p>
    </div></div>
  );
  return (
    <div className="view-container">
      <div className="card login-card">
        ...
        <input className="select-input" type="password" placeholder="Confirmar nova senha"
          value={confirmar} onChange={e=>{setConfirmar(e.target.value);setErr("");}} onKeyDown={e=>e.key==="Enter"&&handleTrocar()}/>
        {err&&<p className="error-msg">⚠️ {err}</p>}
        <button className="btn-primary btn-full" onClick={handleTrocar} disabled={!selected||!senhaAtual||!novaSenha||!confirmar}>Salvar nova senha</button>
  ...
  return (
    <div className="view-container">
      <div className="section-header">
        <div><h2 className="section-title">🏅 Ranking</h2>
          {ultimaAtualizacao&&<p className="section-sub">Atualizado às {ultimaAtualizacao} · {apurados} jogos apurados</p>}
        </div>
        <span className="section-badge">{apurados} jogos apurados</span>
      </div>
      <div className="ranking-table">
        <div className="ranking-header">
          <span>Pos</span><span>Participante</span><span>Pts</span>
          <span className="hide-mobile">Exatos</span><span className="hide-mobile">Acertos</span>
        </div>
        {ranking.map((p,i)=>(
          ...
          {Object.entries(MULTIPLICADORES).map(([fase,m])=>(
            <div key={fase} className="legenda-item">
              ...
              <span className="legenda-pts">
                <span className="pts-exact">{3*m}pts</span> exato · <span className="pts-saldo">{2*m}pts</span> saldo · <span className="pts-winner">{1*m}pts</span> vencedor
              </span>
            </div>
          ))}
        </div>
...
function PalpitesView({participante,palpites,setPalpites,resultados,isAdmin,setResultados,setView}) {
  const [rodadaFiltro,setRodadaFiltro]=useState("Grupos"); const [saved,setSaved]=useState(false);
  const customs=loadStorage(KEY_MINHAS_SENS,{}); const temSenhaCustom=!!customs[participante];
  ...
  function handlePalpite(id,campo,valor,hora){
    if(!podeApostar(hora))return;
    const v=valor.replace(/[^0-9]/g,"").slice(0,2);
    setPalpites(prev=>{const next={...prev,[participante]:{...prev[participante],[id]:{...prev[participante]?.[id],[campo]:v}}};localStorage.setItem(KEY_PALPITES,JSON.stringify(next));return next;});
    setSaved(false);
  }
  function handleResultado(id,campo,valor){
    const v=valor.replace(/[^0-9]/g,"").slice(0,2);
    setResultados(prev=>{const next={...prev,[id]:{...prev[id],[campo]:v}};localStorage.setItem(KEY_RESULTADOS,JSON.stringify(next));return next;});
  }
  function salvar(){localStorage.setItem(KEY_PALPITES,JSON.stringify(palpites));setSaved(true);setTimeout(()=>setSaved(false),3000);}
  return (
    <div className="view-container">
      <div className="section-header">
        <div><h2 className="section-title">🎯 Palpites</h2>
          <p className="section-sub">{participante} · {totalPreenchidos}/{JOGOS.length} preenchidos</p></div>
        <div className="header-actions">
          {!temSenhaCustom&&<button className="btn-trocar-senha" onClick={()=>setView("trocar-senha")}>🔑 Criar senha</button>}
          <button className={`btn-save ${saved?"saved":""}`} onClick={salvar}>{saved?"✓ Salvo!":"💾 Salvar"}</button>
        </div>
      </div>
      {!temSenhaCustom&&(
        <div className="aviso-senha">🔒 <strong>Crie sua senha pessoal!</strong> Qualquer um que saiba seu número de camisa pode acessar sua conta.
          <button className="aviso-btn" onClick={()=>setView("trocar-senha")}>Criar senha agora →</button>
        </div>
      )}
      <div className="progress-bar"><div className="progress-fill" style={{width:`${(totalPreenchidos/JOGOS.length)*100}%`}}/></div>
      <div className="rodada-tabs">
        {rodadas.map(r=><button key={r} className={`rodada-tab ${rodadaFiltro===r?"active":""}`} onClick={()=>setRodadaFiltro(r)}>{getRodadaIcon(r)} {r}</button>)}
      </div>
      <div className="jogos-list">
        {jogosFiltrados.map(jogo=>{
          ...
          const pode=podeApostar(jogo.hora); const tempo=tempoRestante(jogo.hora);
          ...
          return (
            <div key={jogo.id} className={`jogo-card ${preenchido?"filled":""} ${!pode?"locked":""} ${pts!==null?`result-${pts>0?"hit":"miss"}`:""}`}>
              <div className="jogo-meta">
                <span className="jogo-data">{jogo.data}{jogo.hora?` · ${jogo.hora.slice(11,16)}h`:""}</span>
              ...
              <div className="jogo-body">
                ...
                <div className="jogo-placar">
                  <input className="placar-input" type="text" inputMode="numeric" maxLength={2} value={pal.casa}
                    onChange={e=>handlePalpite(jogo.id,"casa",e.target.value,jogo.hora)} placeholder="–" disabled={!pode}/>
                  <span className="placar-x">×</span>
                  <input className="placar-input" type="text" inputMode="numeric" maxLength={2} value={pal.fora}
                    onChange={e=>handlePalpite(jogo.id,"fora",e.target.value,jogo.hora)} placeholder="–" disabled={!pode}/>
                </div>
...
function AdminPanel({setView}) {
  ...
  const [novoNome,setNovoNome]=useState(""); const [novaSenha,setNovaSenha]=useState("");
  const [err,setErr]=useState(""); const [sucesso,setSucesso]=useState("");
  const todos=getParticipantes();
  function adicionar(){
    const nome=novoNome.trim(), senha=novaSenha.trim();
    if(!nome){setErr("Digite o nome.");return;}
    if(!senha){setErr("Digite a senha.");return;}
    if(todos.includes(nome)){setErr("Esse nome já existe!");return;}
    const novos=[...extras,{nome,senha}];
    localStorage.setItem(KEY_EXTRA_PARTS,JSON.stringify(novos));
    setExtras(novos); setNovoNome(""); setNovaSenha(""); setErr("");
    setSucesso(`✅ ${nome} adicionado!`); setTimeout(()=>setSucesso(""),3000);
  }
  function remover(nome){
    if(!window.confirm(`Remover ${nome}?`))return;
    const novos=extras.filter(e=>e.nome!==nome);
    localStorage.setItem(KEY_EXTRA_PARTS,JSON.stringify(novos)); setExtras(novos);
  }
  function resetar(nome){
    const customs=loadStorage(KEY_MINHAS_SENS,{}); delete customs[nome];
    localStorage.setItem(KEY_MINHAS_SENS,JSON.stringify(customs));
    setSucesso(`🔑 Senha de ${nome.split(" ")[0]} resetada!`); setTimeout(()=>setSucesso(""),3000);
  }
  return (
    <div className="view-container">
      <div className="section-header">
      ...
      <div className="admin-card">
        ...
        <div className="admin-form">
          <input className="select-input" type="text" placeholder="Nome completo" value={novoNome} onChange={e=>{setNovoNome(e.target.value);setErr("");}}/>
          <input className="select-input" type="text" placeholder="Senha inicial" value={novaSenha} onChange={e=>{setNovaSenha(e.target.value);setErr("");}} onKeyDown={e=>e.key==="Enter"&&adicionar()}/>
          {err&&<p className="error-msg">⚠️ {err}</p>}
      ...
      {extras.length>0&&(
        <div className="admin-card">
          <h3 className="admin-card-title">👥 Participantes Adicionados</h3>
          <div className="admin-lista">
            {extras.map(e=>(
              <div key={e.nome} className="admin-item">
                ...
                <button className="admin-item-btn" onClick={()=>remover(e.nome)}>🗑️ Remover</button>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="admin-card">
        <h3 className="admin-card-title">🔑 Resetar Senha</h3>
        <p className="admin-card-sub">Quando alguém esquecer a senha. Volta para o padrão.</p>
        <div className="admin-lista">
          {todos.map(nome=>{
            const customs=loadStorage(KEY_MINHAS_SENS,{}); const temCustom=!!customs[nome];
            return (
              <div key={nome} className="admin-item">
                <span className="admin-item-nome">{nome}</span>
                <span className={`admin-item-status ${temCustom?"custom":"padrao"}`}>{temCustom?"🔒 custom":"🔓 padrão"}</span>
                {temCustom&&<button className="admin-item-btn reset" onClick={()=>resetar(nome)}>↺ Resetar</button>}
              </div>
            );
          })}
        </div>
...
function AdminLogin({setIsAdmin,setView}) {
  ...
  function login(){if(pwd===ADMIN_PWD){localStorage.setItem(KEY_ADMIN,"true");setIsAdmin(true);setView("admin-panel");}else{setErr(true);setTimeout(()=>setErr(false),2000);}}
  return (
    <div className="view-container"><div className="card login-card">
      <div className="card-icon">⚙️</div><h2 className="card-title">Área do Organizador</h2>
      <input className="select-input" type="password" placeholder="Senha do organizador" value={pwd} onChange={e=>setPwd(e.target.value)} onKeyDown={e=>e.key==="Enter"&&login()}/>
...
export default function App() {
  ...
  const [participante,setParticipante]=useState(()=>{const s=loadStorage(KEY_SESSION,null);return s?.nome||null;});
  ...
  const fetchResultados=useCallback(async()=>{
    const agora=Date.now(); const ultimo=parseInt(localStorage.getItem(KEY_LAST_FETCH)||"0");
    ...
    if(novos&&Object.keys(novos).length>0){
      setResultados(prev=>{const merged={...prev,...novos};localStorage.setItem(KEY_RESULTADOS,JSON.stringify(merged));return merged;});
  ...
  useEffect(()=>{fetchResultados();const i=setInterval(fetchResultados,FETCH_INTERVAL);return()=>clearInterval(i);},[fetchResultados]);

  function handleLogoClick(){const n=logoClicks+1;setLogoClicks(n);if(n>=5){setShowAdmin(true);setLogoClicks(0);}setTimeout(()=>setLogoClicks(0),3000);}

  if(showAdmin) return (
    <><Header view="admin" setView={()=>setShowAdmin(false)} participante={participante}/>
