...
// ── Senhas custom (salvas no banco para o admin poder resetar) ────────────────
...
export async function resetarSenhaDB(participante) {
  const { error } = await supabase.from("senhas")
    .delete().eq("participante", participante);
  if (error) console.warn("Erro ao resetar senha:", error.message);
  return !error;
}

export async function verificarSenhaDB(nome, senhaDigitada, senhasDB, senhasPadrao, extras) {
  // 1. Verifica senha no banco (custom)
  if (senhasDB[nome]) return senhasDB[nome] === senhaDigitada;
  // 2. Verifica extras adicionados pelo admin
  const extra = extras.find(e => e.nome === nome);
  if (extra) return extra.senha === senhaDigitada;
  // 3. Senha padrão (número da camisa)
  return senhasPadrao[nome] === senhaDigitada;
}
